...
module.exports = function () {
	return {
		...
		calcTemperature: function(hiByte, loByte) {
			...
			return Math.round((((((hiByte & 0x7F) << 8) + loByte) / 10) + 273.15) * 100) / 100;
		},
		...
		calcCRC: function(buffer) {
			// per http://www.switchdoc.com/wp-content/uploads/2015/07/AM2315-3.pdf
			return CRC.crc16modbus(new Buffer([buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5]]));
		},
		convertKelvinToCelsius: function(input) {
			...
			if (input.temperature !== undefined && input.temperatureUnit !== undefined) {
				...
				if (output.temperatureUnit === 'K') {
					...
					output.temperature = Math.round((output.temperature - 273.15) * 100) / 100;
				}
			} else {
				// convert value
		...
		convertKelvinToFahrenheit: function(input) {
			...
			if (input.temperature !== undefined && input.temperatureUnit !== undefined) {
				...
				if (output.temperatureUnit === 'K') {
					...
					output.temperature = Math.round(((output.temperature * 9/5) - 459.67) * 100) / 100;
				}
			} else {
				// convert value
		...
		convertFahrenheitToCelsius: function(input) {
			...
			if (input.temperature !== undefined && input.temperatureUnit !== undefined) {
				...
				if (output.temperatureUnit === '°F') {
					output.temperatureUnit = '°C';
					output.temperature = Math.round(((output.temperature - 32) * (5/9)) * 100) / 100;
				}
			} else {
				// convert value
		...
		convertFahrenheitToKelvin: function(input) {
			...
			if (input.temperature !== undefined && input.temperatureUnit !== undefined) {
				...
				if (output.temperatureUnit === '°F') {
					output.temperatureUnit = 'K';
					output.temperature = Math.round(((output.temperature + 459.67) * (5/9)) * 100) / 100;
				}
			} else {
				// convert value
		...
		convertCelsiusToFahrenheit: function(input) {
			...
			if (input.temperature !== undefined && input.temperatureUnit !== undefined) {
				...
				if (output.temperatureUnit === '°C') {
					output.temperatureUnit = '°F';
					output.temperature = Math.round(((output.temperature * (9/5)) + 32) * 100) / 100;
				}
			} else {
				// convert value
		...
		read: function(callback) {
			...
			var i2cBus = I2C.open(that.config.i2cBusNumber, function(err) {
				if (err) {
					// log error
					if (that.config.debugMode) { console.error(err, err.stack); }
					...
					i2cBus.scan(function(err, devices) {
						if (err) {
							...
							setTimeout(function() {
								...
								i2cBus.i2cWrite(that.config.deviceAddress, 3, new Buffer([that.config.readCmdRegister, 0x00, 0x04]), function(err, bytesWritten, buffer) {
									if (err) {
										// log error
										if (that.config.debugMode) { console.error(err, err.stack); }
										...
										setTimeout(function() {
											// read from the bus
											i2cBus.i2cRead(that.config.deviceAddress, 8, new Buffer(8), function(err, bytesRead, buffer) {
												if (err) {
													// log error
													if (that.config.debugMode) { console.error(err, err.stack); }
